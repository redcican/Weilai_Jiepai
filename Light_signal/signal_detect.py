...
def detect_signal_color_from_frame(
    img: np.ndarray,
    roi: list[int] | None = None,
    signal_center: list[int] | None = None,
    debug: bool = False,
) -> dict:
    """Detect signal light color from a BGR image array.
    ...
    if img is None or img.size == 0:
        return {"color": "unknown", "confidence": 0.0, "reason": "empty image"}
...
def detect_signal_color(
    ...
    """Detect signal light color from a railway camera image file.
    ...
    return detect_signal_color_from_frame(img, roi=roi, signal_center=signal_center, debug=debug)


def detect_signal_video(
    video_path: str,
    roi: list[int] | None = None,
    signal_center: list[int] | None = None,
    debug: bool = False,
    ...
    """Detect signal light color from a video file (sample 1 fps).

    Args:
        video_path: Path to the video file.
        roi: [x1, y1, x2, y2] region of interest.
        debug: If True, save annotated frame images.
        output_dir: Directory to save debug frames.

    Returns:
        dict with video-level summary and per-frame results.
    """
    cap = cv2.VideoCapture(video_path)
    if not cap.isOpened():
        return {"color": "unknown", "confidence": 0.0, "reason": "cannot open video"}

    fps = cap.get(cv2.CAP_PROP_FPS)
    total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
    sample_interval_sec = 1
    max_samples = 60
    frame_interval = max(1, int(fps * sample_interval_sec)) if fps > 0 else 1
    # Cap total samples for very long videos
    if total_frames / frame_interval > max_samples:
        frame_interval = max(1, int(total_frames / max_samples))

    results = []
    frame_idx = 0
    sampled = 0

    while sampled < max_samples and frame_idx < total_frames:
        ...
        res = detect_signal_color_from_frame(
            frame, roi=roi, signal_center=signal_center, debug=debug
        )
        res["frame"] = frame_idx
        res["timestamp"] = round(timestamp, 2)

        if debug and output_dir:
            video_debug_dir = output_dir / Path(video_path).stem
            video_debug_dir.mkdir(parents=True, exist_ok=True)
            if "debug_img" in res:
                out_name = f"f{frame_idx}.jpg"
                cv2.imwrite(str(video_debug_dir / out_name), res.pop("debug_img"))
    ...
    # Summarize
    colors = [r["color"] for r in results if r["color"] != "unknown"]
    color_counts = {}
    for c in colors:
    ...
    return {
        "color": dominant,
        "confidence": round(color_counts.get(dominant, 0) / len(results), 3) if results else 0.0,
...
def _resolve_camera_config(folder: Path, config: dict) -> dict:
    """Walk up directory tree to find matching camera config."""
    current = folder
...
def evaluate_folder(folder: Path, config: dict, debug: bool = False) -> dict:
    """Evaluate all images and videos in a folder against ground truth."""
    gt = load_ground_truth(folder)
    images = [p for p in (sorted(folder.glob("*.png")) + sorted(folder.glob("*.jpg")) + sorted(folder.glob("*.jpeg"))) if not p.name.endswith("_preview.jpg")]
    videos = sorted(folder.glob("*.mp4")) + sorted(folder.glob("*.avi")) + sorted(folder.glob("*.mov"))

    cam_cfg = _resolve_camera_config(folder, config)
    roi = cam_cfg.get("roi")
    ...
    for video_path in videos:
        res = detect_signal_video(str(video_path), roi=roi, signal_center=signal_center, debug=debug, output_dir=debug_dir)
        expected = gt.get(video_path.name)
        predicted = res["color"]
        ...
        results.append({
            ...
            "status": status,
            "confidence": res["confidence"],
            "scores": res.get("color_distribution"),
            "frame_results": res.get("frame_results", []),
        })
...
def main():
    ...
    # --- Detection mode ---
    targets = []
    media_exts = {".png", ".jpg", ".jpeg", ".mp4", ".avi", ".mov"}
    ...
    if args.paths:
        ...
        # Recursive discovery of media-containing directories
        for f in Path(".").rglob("*"):
            if f.is_file() and f.suffix.lower() in media_exts:
                targets.append(("dir", f.parent))
        # Deduplicate and filter
        seen = set()
        unique_targets = []
        for kind, path in targets:
            rp = path.resolve()
            if rp not in seen and not any(part.startswith(".") for part in path.parts) and path.name != "debug":
                seen.add(rp)
                unique_targets.append((kind, path))
    ...
    for kind, path in targets:
        if kind == "file":
            cam_cfg = _resolve_camera_config(path.parent, config)
            roi = cam_cfg.get("roi")
            sc = cam_cfg.get("signal_center")
            if path.suffix.lower() in (".mp4", ".avi", ".mov"):
                res = detect_signal_video(str(path), roi=roi, signal_center=sc, debug=args.debug, output_dir=Path("debug"))
                print(f"  {path.name}: {res['color']} ({res['confidence']:.0%}) [video {res['sampled_frames']} frames]")
                if res.get("frame_results"):
                    print(f"    {'Frame':<8} {'Time':<10} {'Predicted':<10} {'Conf':>6}")
                    for fr in res["frame_results"]:
                        print(f"    {fr['frame']:<8} {fr['timestamp']:>8.1f}s   {fr['color']:<10} {fr['confidence']:>5.0%}")
                    print(f"    {'-' * 40}")
            else:
            ...
            for r in report["results"]:
                ...
                frame_results = r.get("frame_results", [])
                if frame_results:
                    print(f"    {'Frame':<8} {'Time':<10} {'Predicted':<10} {'Conf':>6}")
...
def _discover_folders(paths: list[str]) -> list[Path]:
    """Discover image folders from CLI paths or CWD (recursive)."""
    media_exts = {".png", ".jpg", ".jpeg", ".mp4", ".avi", ".mov"}
